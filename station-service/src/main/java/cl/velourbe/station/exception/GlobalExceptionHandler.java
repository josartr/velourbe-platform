...
/**
 * Global exception handler for the station service.
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleStationNotFound(StationNotFoundException ex) {
        log.warn("Estación no encontrada: {}", ex.getMessage());
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(new ErrorResponse(404, ex.getMessage(), LocalDateTime.now()));
    }

    @ExceptionHandler(InvalidStationException.class)
    public ResponseEntity<ErrorResponse> handleInvalidStation(InvalidStationException ex) {
